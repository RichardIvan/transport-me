...
module.exports = (host, pathnameInfo) => {
  ...
  const dataRequest = `http://${host}/data/`
  const routesRequest = `http://${host}/routes/`
  ...
  const promises = _.map(requests, (request) => {
    ...
    console.log(request)
    return fetch(request).then((response) => response.json()).catch((err) => {
      console.log(err)
    })
